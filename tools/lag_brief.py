...
def beskriv(k):
    ...
    if k.startswith('blob_'):
        ...
        return {'flue': 'a fat bluebottle meat fly with big red eyes', 'svulst': 'a walking pink tumor with veins, one eye and a grin', 'lunge': 'a pink human lung walking on its own, tar spots, tiny sad face', 'yngel': 'drain spawn: a small purple slime creature with mismatched eyes and a toothy grin'}.get(typ, typ) + '. ' + VIEW.get(v, v)
    ...
    for pre, hva in (('hode_', 'HEAD ONLY (no neck, no body)'), ('kropp_', 'TORSO AND HIPS ONLY (no head, no arms, no legs)'), ('kappe_', 'the long black cape with purple lining ALONE, hanging')):
        if k.startswith(pre):
            _, typ, v = k.split('_'); return f'{hva} of {FIG.get(typ, typ)}. {VIEW[v]}'
    return k
...
RUNDER = [
  ('Runde 1: kuriositetene (Isaac-gjenstander)', lambda k: k.startswith(('kur_', 'pille_')) or k == 'glass_tomt', 'Gjenstandene man plukker opp og kombinerer. Små, tydelige og groteske, som gjenstandene i The Binding of Isaac. Bruk gjerne «ni ting»-arket fra DESIGN_BRIEF.md.'),
  ...
  ('Runde 3: rekvisitter og møbler', lambda k: k.startswith(('prop_', 'seng_', 'skap', 'hylle', 'disk_', 'kiste_')) or k in ('journalskap', 'olsenskap', 'pult', 'sjakt', 'alter3', 'kommodef', 'garderobes', 'vaskemaskin2', 'sperre', 'lik'), 'Møblene står i 3/4-vinkel: du ser fronten og litt av toppen. Bunnen av møbelet helt nederst i motivet.'),
  ('Runde 4: plukk, effekter og tillegg', lambda k: k in ('hjerte', 'morbdrape', 'tann', 'due_figur', 'stempelmerke', 'skyggehand_p', 'stjerne', 'skudd_slim') or k.startswith(('flaske_', 'puff', 'tillegg_')), 'Små ting. Enkle former, tydelig kontur.'),
...
for tittel, f, merk in RUNDER:
    ...
    ut += [f'## {tittel} ({len(ks)} bilder)', '', merk, '', '| Filnavn | Beskrivelse til ChatGPT | Format |', '|---|---|---|']
    ut += [f'| `{k}.png` | {beskriv(k)} | {format_(man[k])} |' for k in ks]
    ut.append('')
rest = sorted(k for k in man if k not in brukt)
if rest: ut += ['## Øvrige', '', '| Filnavn | Beskrivelse | Format |', '|---|---|---|'] + [f'| `{k}.png` | {beskriv(k)} | {format_(man[k])} |' for k in rest] + ['']

...
const HANDBOK = [
  { id: 'velkommen', t: 'Velkommen', note: 'De sier det er fire etasjer. Jeg har talt fem.', art: () => portraitOf(G.run && G.player ? G.run.look : Pasient.lag(), 150, 190),
    b: `<p>Du er innlagt ved Morbidium sanatorium. Det finnes ingen vei ut, bare ned.</p>
      <p>Bygget har <b>fire etasjer</b>. I hver av dem holder en <b>overlege</b> til bak en låst dør. Behandle overlegen, så åpner luka ned til neste etasje seg. Under grunnmuren venter noe som kaller seg Journalen.</p>
  ...
  { id: 'morbidium', t: 'Morbidium', note: 'Hører du hvisking, er det ikke meg.', art: () => canvasOf(morbPart(), 100, 130),
    b: `<p>Den lilla stripen er <b>Morbidium</b>, mørket i blodet. Det fylles av dråper, forbannelser, visse kort og visse handler.</p>
      <ul><li><b>Over 55:</b> noe hvisker.</li><li><b>Over 60:</b> hikke som kaster deg til siden.</li><li><b>Over 75:</b> du ser ting som ikke er der, men evnene lades 15 % raskere.</li><li><b>100:</b> mørket spiser helsa.</li></ul>
...
function openHandbook(o = {}, kap = 0) {
  show('title', false);
  const K = HANDBOK[kap], smal = narrow();
  openPanel(`<div class="fit hefte${smal ? ' smal' : ''}"><div class="hside"><div class="htitle">Pasient&shy;håndbok</div><div class="hsub">for innlagte ved Morbidium sanatorium<br>utgave 1923</div>
      <div class="htabs">${HANDBOK.map((h, i) => `<button class="htab${i === kap ? ' on' : ''}" data-kap="${i}"><i>${i + 1}</i>${h.t}</button>`).join('')}</div></div>
    <div class="hpage"><div class="hkap">Kapittel ${kap + 1}</div><h2>${K.t}</h2><div class="hcols"><div class="htext">${typeof K.b === 'function' ? K.b() : K.b}</div>${K.art ? '<div class="hart"><span id="hArt"></span><div class="hnote">' + esc(K.note) + '</div></div>' : ''}</div>
      ${K.art ? '' : '<div class="hnote solo">' + esc(K.note) + '</div>'}
      <div class="hnav"><button class="btn" id="hPrev" ${kap ? '' : 'disabled'}>Forrige</button><span>Side ${kap + 1} av ${HANDBOK.length}</span><button class="btn" id="hNext" ${kap < HANDBOK.length - 1 ? '' : 'disabled'}>Neste</button><button class="btn big" data-close>Lukk</button></div></div></div>`, Object.assign({}, o, { refit: () => openHandbook(o, kap) }));
  if (K.art) { try { place('#hArt', K.art()); } catch (e) { } }
  const go = k => { Sound.play('paper'); openHandbook(o, k); };
  document.querySelectorAll('[data-kap]').forEach(b => b.onclick = () => go(+b.dataset.kap));
  $('hPrev').onclick = () => kap && go(kap - 1); $('hNext').onclick = () => kap < HANDBOK.length - 1 && go(kap + 1);
  G.handbokKap = kap; fitPanel();
}

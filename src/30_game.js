...
function loop(now) {
  ...
  Musikk.tick(); Musikk.dempet(G.state === 'panel' || G.state === 'journal'); D3.tick(dt); Effekter.tick(dt); Dybde.tick(dt);
  if (G.state === 'play') {
    const P = G.player;

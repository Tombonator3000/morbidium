...
const Merknad = {
  ...
  onBoss(B) { const id = { krok: 'krok', rust: 'rust', arkivar: 'arkivar' }[B.type]; if (id) this.gi(id); },
  onWin(run) { this.gi('utskrevet'); if ((performance.now() - run.t0) / 60000 < 30) this.gi('rask'); if (run.gjen) this.gi('gjen'); },
  onKill(e) { const m = G.meta; m.drap = (m.drap || 0) + 1; if (e.mesterNavn) m.mestere = (m.mestere || 0) + 1; if (m.drap >= 100) this.gi('drap100'); if ((m.mestere || 0) >= 5) this.gi('mester5'); },

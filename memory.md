...
- Liste 10 (Historien) har journalsidene og sluttbildene på ett ark og forstanderen som eget bilde. Runde 14 i ART_BRIEF.md.
...
- Hovedhistorien: Morbidium sanatorium ble grunnlagt i 1887 av forstander dr. Mathias Morbeck («M.» i journalen). Han kjøpte en journal i svart skinn for to kroner og femti for å skrive ned alle sinn i huset, og journalen begynte å skrive selv. Morbidium er blekket den skriver med, laget av det pasientene ikke sier høyt. Bygget vokser nedover (kjelleren 1901, skogen høsten 1918). Pasienten er den samme sjela under nye navn, og slipper ut ved å lese sin egen journal ferdig. Spillet foregår 24. september 1923, så ingen har jobbet der i mer enn trettiseks år.
...
- Forstanderen (HENDELSER.forstanderen, bare i Dypet, vekt 60, rom:*): spør, les over skulderen (linjer fra historien, mer jo flere drømmer), ta pennen (G.run.pennen, sjefSvekk[6] = .2, 15 Morbidium, merknaden Pennen) eller la ham lese (25 helse). G.run.forstanderMott settes når han er snakket med.
- Sjefene: LINES.monolog2 (ny tale ved en tredjedel helse, byttes inn i bossOnHurt), slengord i kampen hvert 8. til 14. sekund (LINES.boss per type), LINES.bossDod (siste ord, på et merke der sjefen sto fordi FX fjerner bobler på døde). Journalen svarer seg selv etter 2,3 sekunder. Hjorten nevner den hvite hesten hvis tegnet i drømmene var hest.
- Historie.personlig() kjøres etter hver startFloor: setter PA, LINES.koret og NPC_LINES tilbake til utgangspunktet og legger til linjer fra drømmene (fra etasje 3) og NPC_DYP (personalet vet hvor de er, Olsen har en historie om nøklene).
- Slutten: Drom.epilog er pakket inn med «Siste side» (SISTE_SIDE per slutt) før etterordet. Historie.brev() gir tittel, stempel, avslutning og underskrift til utskrivningsbrevet (gjentakelse gir Innkallingsbrev og INNKALT; med pennen signerer pasienten). Historie.konklusjon() står på dødskortet ved utskrivning.
- Drom.historie er pakket inn så personen passer til alderen (ingen bestefar over 45, ingen mor eller far over 58, ikke barn under 22).
- Seks nye journalfragmenter (LORE, lagt til på slutten, så gamle indekser står), og merknadene Løsrevet, Villnis, Ansiktet tilbake og Pennen.
- Bildene (HISTORIE_ART): historie_1 til 5, historie_slutt_<slutt> og prop_forstander, med reservetegninger i koden. Liste 10 i tegnelister/.
